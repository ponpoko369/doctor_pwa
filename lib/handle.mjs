...
// 5-15s. We send a brief "estoy revisando" ack synchronously *before* the
// Claude call so the patient sees something within ~1s. If the Edge isolate
// gets killed mid-Claude-call (waitUntil isn't reliable on every Vercel
// config), the patient still has the ack — not silence. They can re-prompt
// if needed and the dedup keeps that from snowballing.
